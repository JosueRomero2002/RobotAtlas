...
class ESP32Client:
    ...
    def send_movement(self, bi: int, bd: int, fi: int, fd: int, hi: int, hd: int, pd: int) -> bool:
        """
        ...
        """
        try:
            # Crear parámetros para el comando BRAZOS
            params = f"BI={bi} BD={bd} FI={fi} FD={fd} HI={hi} HD={hd} PD={pd}"
...
def send_esp32_movement(bi: int, bd: int, fi: int, fd: int, hi: int, hd: int, pd: int) -> bool:
    """
    ...
    """
    client = get_esp32_client()
    return client.send_movement(bi, bd, fi, fd, hi, hd, pd)
