...
#  UI/ANIMATION FLAGS
# ======================
# Controla si se muestra la ventana con el dibujo de la cara
SHOW_FACE_ANIMATION = False
# ======================
#  RUTAS DE LOS QR
# ======================
QR_PATHS = {
    ...
    try:
        ...
        with fitz.open(pdf_path) as doc:
            ...
            while slide_num < total_slides and exit_flag.value == 0:
                ...
                if slide_num % 6 == 0 and slide_num < total_slides:
...
def esp32_action_resolver(sequence_name: str) -> bool:
    ...
    try:
        ...
        try:
            ...
            sequences_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "sequences")
            sequence_file = None
        ...
        try:
            ...
            for i, action in enumerate(actions):
                ...
                if command == "BRAZOS":
                    ...
                    response = esp32_client.send_movement(bi, bd, fi, fd, hi, hd, pd)
                    ...
                    # Enviar comando de gesto al ESP32
                    response = esp32_client.send_gesture(gesture)
        ...
        with fitz.open(pdf_path) as doc:
            ...
            while slide_num < total_slides and exit_flag.value == 0:
                ...
                if page_text.strip():
                    ...
                    El siguiente texto es de una diapositiva de clase sobre robótica. 
                    No resumas simplemente el contenido, sino explícalo como lo haría un profesor 
                    entusiasta en clase pero hacerlo CONCISO máximo (1-2 frases),
                    añadiendo contexto y haciéndolo interesante y conversacional,
                    no digas que imagine, solo explica el contenido,
                    estas siguiendo una secuencia de pasos para realizar el experimento.
                    :
...
def main():
    ...
    try:
        ...
        # Crear ventana para la cara animada (opcional)
        if SHOW_FACE_ANIMATION:
        ...
        if explain_slides_with_random_questions(engine, pdf_path, pdf_text, current_users,
            ...
            sequence_mapping = {
             ...
             # PDF
            demo_pdf_path = os.path.join(script_dir, "pdfs/Demo_Paso_a_Paso.pdf")
            demo_pdf_text = extract_text_from_pdf(demo_pdf_path)


            if explain_slides_with_sequences(engine, demo_pdf_path, demo_pdf_text, current_users,
                                        hand_raised_counter, current_slide_num, exit_flag, 
